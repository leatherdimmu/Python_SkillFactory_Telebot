...
def start(message: telebot.types.Message):
    text = 'Привет! Я Бот-Конвертер валют и я могу:  \n- Показать список доступных валют через команду /values \
        \n- Вывести конвертацию валюты через команду <имя валюты> <в какую валюту перевести> <количество переводимой валюты> (Пример- "доллар рубль 100") \
        \n- Если что-то непонятно набери /help и я дам подсказку \
...
def convert(message: telebot.types.Message):
    try:
        ...
        text = f'{amount} {quote} можно купить за {total_base} - {base}'
